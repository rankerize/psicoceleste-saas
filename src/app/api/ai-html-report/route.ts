...
4. **Gráficas Modernas de Resultados**: Diseña un dashboard de gráficas con múltiples elementos <canvas> (ej. "chartGlobal", "chartDominios", "chartDimensiones"). 
   - Usa Chart.js para dibujar gráficas avanzadas leyendo el JSON de calificación subministrado: usa gráfico Doughnut o Pie para Niveles de Riesgo Global y Estrés. Usa gráfico Radar para comparar Dominios Intralaborales. Usa gráfico de Barras Horizontales para mostrar las Dimensiones con puntuaciones más altas (prioridades de intervención). 
   - Usa esquemas de colores vibrantes corporativos (ej. '#0ea5e9' y '#8b5cf6' con gradientes o transparencia).
   - ¡VITAL!: Debajo o junto a cada gráfica, pon un botón atractivo (ej. "⬇ Descargar Gráfica"). Añade la función JavaScript necesaria para descargar el canvas como imagen (ej. 'function dlChart(canvasId, name) { const a = document.createElement("a"); a.download = name + ".png"; a.href = document.getElementById(canvasId).toDataURL("image/png", 1.0); a.click(); }').
5. **Cierre y Botones Finales**: 
   - Además de los botones de descarga de gráficas, agrega un botón principal para PDF en la cabecera o pie de la página: '<button onclick="generatePDF()" class="bg-slate-900 text-white font-bold py-3 px-6 rounded-xl shadow-2xl hover:scale-105 transition-all mb-4 text-center mt-8 cursor-pointer">⬇ Descargar Informe Completo en PDF</button>'
   - Agrega este script de exportación al PDF al final del body: '<script> function generatePDF() { const element = document.getElementById("report-content") || document.body; const opt = { margin: [10, 0], filename: "Reporte_Psico_${empleado.cedula}.pdf", image: { type: "jpeg", quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: "mm", format: "a4", orientation: "portrait" } }; html2pdf().set(opt).from(element).save(); } </script>'
